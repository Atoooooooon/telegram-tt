...
为所有通过过滤的有用消息建立可追踪、可升级、可回查的处理闭环,确保它们满足以下约束:

1. 每条有用消息都必须进入一个明确的处理状态
...
当前 Web 端的核心职责不是给人浏览所有消息,而是为过滤后的有用消息提供漏单保护。
...
因此,应该将“有用消息”聚合为“待处理事项(case)”。

### 3.3 保障逻辑必须显式建模

一条有用消息只要通过过滤,就不应该停留在“存在于列表里”这个模糊状态,而应具备:
...
> 任何一条有用消息,在约定时间内,要么有人确认接手,要么系统自动升级。
...
> 任意一条通过过滤的有用消息,在 N 分钟内,要么被明确接手,要么被系统自动升级。
...
当前系统已经解决了“噪音太多”的问题,下一阶段应解决“有用消息没人接住”的问题。

本方案的核心不是继续优化过滤器,而是为过滤后的有用消息补上以下能力:
